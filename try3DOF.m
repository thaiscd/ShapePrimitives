function [bool, o, r0, r1, r2, theta] = try3DOF(nitermax, MPs, Obs, OC, maxlength, n)
...
nonlcon = @(x) constraints3DOF(x, MPs, Obs, OC);
...
while bool ~= 1 && indx <= nitermax 
    ...
    try
        ...
        [c,ceq] = constraints3DOF(x, MPs, Obs, OC);
        c1 = max(c)
        ceq1 = abs(max(ceq))

        if c1 < viol && ceq1 < viol
            o = x(1 : 2)';
            a = x(3 : 2 + 2*nMPs);
            r0 = x(3 + 2*nMPs);
            r1 = x(4 + 2*nMPs);
            ...
            [bool, theta] = TrajectoryCheck(n, MPs, Obs, o, a, r0, r1, r2, bl1, bl2, bc, ba1, ba2);
            if bool == 0
                fun = @(x) minL(x, nL, nMPs);
                nonlcon = @(x) constraints3DOF(x, MPs, Obs, OC);
