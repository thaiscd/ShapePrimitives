function [bool, a, r1, r2, theta] = try2DOF(nitermax, MPs, Obs, OC, maxlength)
...
nonlcon = @(x) constraints2DOF(x, MPs, Obs, OC);
...
while bool ~= 1 && indx <= nitermax
    ...
    try
        ...
        [c,ceq] = constraints2DOF(x, MPs, Obs, OC);

        if all(c <= 1e-3) && all(abs(ceq) < viol)
            a = x(1:2)';
            r1 = x(3);
            r2 = x(4);
            y = x(5:end);
            ...
            [b, theta] = FindThetasMPs2DOF(r1, r2, a, bl1, bl2, bc, ba1, ba2, MPs, n, Obs);
            if b == 0
                bool = 1;
            end
